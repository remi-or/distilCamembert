...
class DistilledCamembertModel(Module):
    ...
    def ce_loss(
        ...
        return torch.nn.NLLLoss(teacher_pooler_outputs * student_pooler_outputs)
    ...
    def batch(
        ...
        cosine_loss = torch.nn.CosineEmbeddingLoss(teacher_outputs[0], student_outputs[0]) / 3
        cross_entropy_loss = self.ce_loss(teacher_outputs[0], student_outputs[0]) / 3
        if problem_type == 'classification':
            specific_loss = torch.nn.CrossEntropyLoss(student_outputs[1], dataset_batch['labels']) / 3
        loss = cosine_loss + cross_entropy_loss + specific_loss
    ...
    def forward_sequence_classification(
        ...
        with torch.no_grad():
            ...
            teacher_outputs = (teacher_pooler_outputs, teacher_classifier_outputs)
        ...
        student_outputs = (student_pooler_outputs, student_classifier_outputs)
